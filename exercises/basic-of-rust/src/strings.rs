...
fn exercise1(color: &str) -> String {
    let string_color: String = color.chars().collect();
    return string_color;
...
fn is_palindrome(word: &str) -> bool {
    let string : String = word.chars().collect();
    let string_rev: String = word.chars().rev().collect();
    if string.to_lowercase() == string_rev.to_lowercase() {
        return true
    }
    return false
}
